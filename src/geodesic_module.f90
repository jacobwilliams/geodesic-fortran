...
module geodesic_module
  ...
  integer,parameter,private :: digits = 53
  integer,parameter,private :: maxit1 = 20
  integer,parameter,private :: maxit2 = maxit1 + digits + 10

  double precision,parameter,private :: dblmin = 0.5d0**1022
  double precision,parameter,private :: dbleps = 0.5d0**(digits-1)

  double precision,parameter,private :: pi = atan2(0d0, -1d0)
  double precision,parameter,private :: degree = pi/180
  ...
  double precision,parameter,private :: tiny = 0.5d0**((1022+1)/3)
  double precision,parameter,private :: tol0 = dbleps
  ...
  double precision,parameter,private :: tol1 = 200 * tol0
  double precision,parameter,private :: tol2 = sqrt(tol0)
  ! Check on bisection interval
  double precision,parameter,private :: tolb = tol0 * tol2
...
double precision coeff(nA1/2 + 2)
...
integer o, m, l
double precision coeff((nC1**2 + 7*nC1 - 2*(nC1/2))/4)
data coeff / &
    -1, 6, -16, 32, &
    -9, 64, -128, 2048, &
    9, -16, 768, &
...
double precision eps2, d
integer o, m, l
double precision coeff((nC1p**2 + 7*nC1p - 2*(nC1p/2))/4)
data coeff / &
    205, -432, 768, 1536, &
...
double precision coeff(nA2/2 + 2)
...
double precision eps2, d
integer o, m, l
double precision coeff((nC2**2 + 7*nC2 - 2*(nC2/2))/4)
data coeff / &
    1, 2, 16, 32, &
    35, 64, 384, 2048, &
    15, 80, 768, &
...
integer o, m, k, j
double precision coeff((nA3**2 + 7*nA3 - 2*(nA3/2))/4)
data coeff / &
    -3, 128, &
    -2, -3, 64, &
    -1, -3, -1, 16, &
...
integer o, m, l, j, k
double precision coeff(((nC3-1)*(nC3**2 + 7*nC3 - 2*(nC3/2)))/8)
data coeff / &
    3, 128, &
    2, 5, 128, &
    -1, 3, 3, 64, &
...
integer o, m, l, j, k
double precision coeff((nC4 * (nC4 + 1) * (nC4 + 5)) / 6)
data coeff / &
...
double precision lon1x, lon2x, lon12, e
